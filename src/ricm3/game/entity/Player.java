	...
	public void pop(long now) {
		if (m_energie >= 3) {
			...
			if (global_map.caseLibre(this.getX(), this.getY(), p)) {
				Minion minion = new Minion(m_model.kamikaze_idle,m_model.kamikaze_droite, p.x, p.y,true,true,true,false, Options.MINION_MS, this.m_model.m_automates.get(m_indiceAutoMinions), Orientation.RIGHT, 1,
